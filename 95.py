...
def fa_digits(n):
    """نمایش عدد با رقم‌های فارسی، بدون تغییر رفتار عددی داخلی."""
...
تبادل راهنما — راهنمای حالت‌های تبادل
تبادل دستورها — فهرست همه دستورهای تبادل
...
class Engine:
    ...
    def exchange_cmd(self, arg):
        ...
        if sub in ("راهنما", "حالت‌ها", "حالت ها", "guide", "modes"):
            return self.exchange_guide_text()
        if sub in ("دستورها", "دستور", "commands", "command", "all"):
            return self.exchange_commands_text()
    ...
    def exchange_text(self):
        ...
        این صفحه عمداً کوتاه و ثابت چیده شده است. نسخه‌ی قبلی هر فرمان را
        با توضیح‌های طولانی پشت سر هم می‌نوشت و در تلگرام شبیه یک متن به‌هم
        ریخته دیده می‌شد. لینک‌های صفحه‌ها هم فرمان واقعی هستند تا کپی‌کردن
        یا فرستادنشان مستقیماً همان صفحه/عملکرد را باز کند.
        """
        x = self.ex_cfg()
        counts = self.db.ex_counts()
        channel = (self.st.prof("standard")["channel"] or "").strip()
        groups = x.get("groups") or []

        status = "🟢 روشن" if x.get("enabled") else "🔴 خاموش"
        auto_join = "روشن" if x.get("auto_join", True) else "تأیید دستی"
        adaptive = "روشن" if x.get("adaptive_on", True) else "خاموش"

        # «انجام‌شده» همان رکوردهای موفق فعلی است و «در صف» فقط Joinهای
        # تأییدشده‌ای است که هنوز توسط worker اجرا نشده‌اند.
        done = counts.get("joined", 0)
        queued = counts.get("approved", 0)

        perm_on = bool(x.get("permanent_check", True))
        perm_hours = int(x.get("permanent_check_max_hours", 24) or 0)
        if not perm_on:
            watch = "خاموش"
        elif perm_hours <= 0:
            watch = "تا ابد"
        else:
            watch = f"تا {fa_digits(perm_hours)} ساعت"

        report = {"live": "لحظه‌ای", "summary": "خلاصه", "off": "خاموش"}.get(
            x.get("report_mode", "live"), "خاموش")
        smart = "روشن" if self.ai.cfg.get("smart_detect", True) else "خاموش"
        gap_min = fa_digits(x.get("min_join_gap_sec", 30) or 30)
        gap_max = fa_digits(x.get("max_join_gap_sec", 60) or 60)
        ...
        lines = [
            ...
            "━━━━━━━━━━━━━━",
            "",
            f"وضعیت: {status} · جوین خودکار: {auto_join}",
            f"کانال من: {channel or 'تنظیم نشده'} · گروه‌ها: {fa_digits(len(groups))}",
            f"جوین امروز: {fa_digits(self.db.ex_joins_today())} · انجام‌شده: {fa_digits(done)} · در صف: {fa_digits(queued)}",
            f"فاصله Join: {gap_min}–{gap_max} ثانیه (تطبیقی {adaptive})",
            f"نگهبانی عضویت: {watch} · گزارش: {report}",
            "",
            "━━━ صفحه‌ها ━━━",
            "",
            ...
            "",
            f"🧠 تشخیص هوشمند: {smart}",
            "",
            "↩️ بازگشت: عادی",
        ]
        return "\n".join(lines)

    def exchange_guide_text(self):
        """راهنمای کوتاهِ حالت‌های تبادل؛ مقصدِ «تبادل راهنما»."""
        x = self.ex_cfg()
        return "\n".join([
            ...
            "",
            "🟢 روشن/خاموش",
            "`تبادل روشن` یا `تبادل خاموش` — کل چرخه‌ی تبادل را فعال یا متوقف می‌کند.",
            "",
            "🤖 جوین خودکار",
            "`تبادل خودکار` — بعد از تأیید عضویت، Join بدون پرسش انجام می‌شود؛ با خاموش‌کردن آن، قبل از Join تأیید دستی لازم است.",
            "",
            "🚶 پیش‌قدم",
            "`تبادل پیش‌قدم` — گروه‌های ثبت‌شده را اسکن می‌کند و لینک‌های تازه را خودش برای شروع تبادل بررسی می‌کند.",
            "",
            "🛡 نگهبانی عضویت",
            "بعد از Join، عضویت طرف دوباره بررسی می‌شود. اگر عدم عضویت چند بار تأیید شود، طبق تنظیمات یادآوری و لفت انجام می‌شود.",
            "",
            "🧠 تطبیقی",
            "فاصله‌ی Join با FloodWait و آپ‌تایم تنظیم می‌شود تا فشار روی حساب کمتر شود.",
            "",
            f"وضعیت فعلی: {'روشن' if x.get('enabled') else 'خاموش'} · جوین خودکار: {'روشن' if x.get('auto_join', True) else 'تأیید دستی'}",
            "",
            "⌨️ جزئیات همه‌ی فرمان‌ها: `تبادل دستورها`",
            "↩️ بازگشت: `تبادل`",
        ])

    def exchange_commands_text(self):
        """فهرست عملیاتی فرمان‌ها؛ مستقل از HELP عمومی و قابل کپی."""
        return "\n".join([
            "⌨️ همه دستورهای تبادل",
            "━━━━━━━━━━━━━━",
            "",
            "تبادل — صفحه‌ی اصلی",
            "تبادل روشن / تبادل خاموش — فعال یا متوقف‌کردن",
            "تبادل خودکار — روشن/خاموش‌کردن جوین خودکار",
            "تبادل پیش‌قدم — روشن/خاموش‌کردن اسکن گروه‌ها",
            "تبادل اسکن — اسکن فوری",
            "تبادل فهرست — فهرست همه‌ی تبادل‌ها",
            "تبادل منتظر — موارد منتظر",
            "تبادل تأیید ۵ — تأیید Join شماره‌ی ۵",
            "تبادل رد ۵ — رد شماره‌ی ۵",
            "تبادل خروج ۵ — لفت از کانال شماره‌ی ۵",
            "تبادل حذف ۵ — حذف شماره‌ی ۵",
            "تبادل ارسال ۵ — ارسال دوباره‌ی پیام Join شماره‌ی ۵",
            "",
            "⚙️ تنظیمات اصلی",
            "تبادل تنظیمات — صفحه‌ی تنظیمات",
            "تبادل فاصله ۳۰ ۶۰ — بازه‌ی فاصله‌ی Join",
            "تبادل بررسی ۱۵ ۳۰ — فاصله‌ی بررسی عضویت",
            "تبادل دائمی — نگهبانی عضویت",
            "تبادل فاصله عملکرد ۱۵ ۲۰ — فاصله‌ی بین عملکردها",
            "تبادل تطبیقی — تنظیم فاصله‌ی هوشمند",
            "تبادل سقف ساعتی — سقف Join در یک ساعت",
            "",
            "💬 متن و گزارش",
            "متن‌های تبادل — متن‌های پاسخ و Join",
            "تبادل گزارش — گزارش خصوصی",
            "تبادل پیام موفق متن شما — متن موفقیت",
            "تبادل پیام ناموفق متن شما — متن نبودن عضویت",
            "",
            "📡 گروه و اسکن",
            "گروه‌ها — فهرست گروه‌ها",
            "افزودن گروه @group — افزودن گروه",
            "تبادل عمق اسکن ۵۰ — تعداد پیام‌های اسکن",
            "تبادل سن لینک ۵ — حداکثر سن لینک به دقیقه",
            "",
            "📖 راهنمای حالت‌ها: `تبادل راهنما`",
            "↩️ بازگشت: `تبادل`",
        ])
...
async def connect_and_run(eng, creds):
    ...
    async def on_saved(event):
        ...
        bare = {
            ...
            "تبادل حالت‌ها": ("ex", "راهنما"),
            "تبادل حالت ها": ("ex", "راهنما"),
            "تبادل دستورها": ("ex", "دستورها"),
            "تبادل دستور": ("ex", "دستورها"),
            "گزارش": ("ex", "report"),
